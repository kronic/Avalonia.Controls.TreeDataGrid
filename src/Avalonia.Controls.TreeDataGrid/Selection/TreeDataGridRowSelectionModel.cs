...
namespace Avalonia.Controls.Selection
{
        ...
        where TModel : class
    {
        ...
        private long _lastCharPressedTime;
        ...
        protected void HandleCharKey(string? text, TreeDataGrid treeDataGrid, int selectedRowIndex)
        {
            if (text != null && treeDataGrid.Columns != null)
            {
                ...
                {
                    if (column is ITextSearchable textSearchableColumn && textSearchableColumn.IsTextSearchEnabled == true)
                    {
                        ...
                        hierarchicalColumn.Inner is ITextSearchable textSearchableColumn2)
                    {
        ...
        private void Search(TreeDataGrid treeDataGrid, string candidatePattern, int selectedRowIndex, ITextSearchable column)
        {
            var textColumn = column as ISearchableTextColumn<TModel>;
            var templateColumn = column as ISearchableTemplateColumn<TModel>;
            var found = false;
            var selector = textColumn == null ? templateColumn!.TextSearchValueSelector : textColumn.ResolveStingValueFromModel;
            for (int i = candidatePattern.Length == 1 ? selectedRowIndex + 1 : selectedRowIndex; i <= _source.Rows.Count - 1; i++)
            {
                found = SearchAndSelectRow(treeDataGrid, candidatePattern, i, (TModel?)_source.Rows[i].Model, selector);
                if (found)
                {
                    break;
                }
            ...
            string candidatePattern, int newIndex, TModel? model, Func<TModel, string?>? valueSelector)
        {
            if (valueSelector != null && model != null)
            {
                ...
                if (value != null && value.ToUpper().StartsWith(candidatePattern.ToUpper()))
                {
                    UpdateSelection(treeDataGrid, newIndex, true);
                    if (treeDataGrid.RowsPresenter != null)
                    {
        ...
        {
            HandleCharKey(e.Text, sender, _source.Rows.ModelIndexToRowIndex(AnchorIndex));
        }
